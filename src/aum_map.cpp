...
// because we use log-time STL map methods (insert, find) err_N
// times. This code is slightly slower than aum_sort (by constant
// factors), but this code is also a bit more user-friendly (temporary
// storage managed by C++ constructors / destructors, etc).
int aum_map
(const double *err_pred, //err_N
 ...
 double *out_deriv_mat //pred_N*2
  ...
  std::vector<double> out_thresh(err_N);
  *out_aum = 0.0;
  ...
  for(int pred_i=0; pred_i<pred_N; pred_i++){
    if(!isfinite(pred_vec[pred_i])){
      return ERROR_AUM_MAP_ALL_PREDICTIONS_SHOULD_BE_FINITE;
    }
  }
  ...
  for(int row=0; row<err_N; row++){
    ...
    if(row_example >= pred_N){
      return ERROR_AUM_MAP_EXAMPLE_SHOULD_BE_LESS_THAN_NUMBER_OF_PREDICTIONS;
    }
    if(row_example < 0){
      return ERROR_AUM_MAP_EXAMPLE_SHOULD_BE_NON_NEGATIVE;
    }
  ...
  for(TotalsMap::iterator it=totals_map.begin();
    ...
    if(cumsum < 0){
      return ERROR_AUM_MAP_FP_SHOULD_BE_NON_NEGATIVE;
    }
  ...
  for(TotalsMap::reverse_iterator it=totals_map.rbegin();
    ...
    if(cumsum < 0){
      return ERROR_AUM_MAP_FN_SHOULD_BE_NON_NEGATIVE;
    }
